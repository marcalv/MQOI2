...
from launcher import  checkSolution, completeSolve, randomSolve
...
for fileIndex in range(1,2):
    ...
    bestSolution=randomSolve(dataFileExample)
    print(bestSolution['totalCost'])
...
with open(os.path.join('outputs', 'statics_report_random'+'.csv'), 'w') as f:
    for line in csv_lines:
        f.write("%s\n" % line)
